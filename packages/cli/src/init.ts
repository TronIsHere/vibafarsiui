...
function pickFont(project: Project, requested: FontId): { id: FontId; cssVar: string; exportName: string } {
...
function ensureImport(source: string, line: string) {
...
function ensureFontModule(project: Project, font: ReturnType<typeof pickFont>, dryRun: boolean) {
  ...
  if (!src.trim()) src = font.id === "iransans" ? iranSansSnippet() : vazirmatnSnippet();
  writeFile(file, src, dryRun);
  return file;
}

function addImport(source: string, spec: string, names: string[]) {
...
function patchHtmlOpen(source: string, classExpr?: string) {
  return source.replace(/<html\b([^>]*)>/, (_, raw: string) => {
    let attrs = raw;
    if (/\blang\s*=/.test(attrs)) attrs = attrs.replace(/\blang\s*=\s*(["']).*?\1/, 'lang="fa"');
    else attrs += ' lang="fa"';
    if (/\bdir\s*=/.test(attrs)) attrs = attrs.replace(/\bdir\s*=\s*(["']).*?\1/, 'dir="rtl"');
    else attrs += ' dir="rtl"';
    if (classExpr) {
      if (/\bclassName\s*=/.test(attrs)) {
        attrs = attrs.replace(/\bclassName\s*=\s*\{`([^`]*)`\}/, (_m, inner: string) => {
          if (inner.includes(classExpr)) return `className={\`${inner}\`}`;
          return `className={\`${classExpr} ${inner}\`}`;
        });
        attrs = attrs.replace(/\bclassName\s*=\s*\{([^}]+)\}/, (m, inner: string) => {
          if (m.includes("`") || inner.includes(classExpr)) return m;
          return `className={\`${classExpr} \${${inner.trim()}}\`}`;
        });
        attrs = attrs.replace(/\bclassName\s*=\s*(["'])([^"']*)\1/, (_m, q: string, inner: string) => {
          if (inner.includes(classExpr)) return `className=${q}${inner}${q}`;
          return `className={\`${classExpr} ${inner}\`}`;
        });
      } else {
...
function patchBodyFont(source: string) {
  return source.replace(/<body\b([^>]*)>/, (m, raw: string) => {
    if (/\bclassName\s*=/.test(raw)) {
      if (/font-sans/.test(raw)) return m;
...
function patchLayout(project: Project, font: ReturnType<typeof pickFont>, fontsFile: string, dryRun: boolean) {
...
function patchIndexHtml(project: Project, dryRun: boolean) {
  const file = project.paths.indexHtml;
  if (!file) return false;
  let src = readText(file);
  if (!src) return false;
  const next = src.replace(/<html\b([^>]*)>/, (_, raw: string) => {
    let attrs = raw;
    if (/\blang\s*=/.test(attrs)) attrs = attrs.replace(/\blang\s*=\s*(["']).*?\1/, 'lang="fa"');
    else attrs += ' lang="fa"';
    if (/\bdir\s*=/.test(attrs)) attrs = attrs.replace(/\bdir\s*=\s*(["']).*?\1/, 'dir="rtl"');
...
async function writeLibFromRegistry(project: Project, flags: Flags, slug: string) {
  const client = makeClient(project.config.registry);
  const catalog = await fetchCatalog(client);
  const item = resolveOne(catalog, slug);
  if (!item) throw new Error(`Registry is missing lib "${slug}"`);
...
export async function runInit(flags: Flags) {
  ...
  try {
    ...
    if (!themeCss) throw new Error(`Theme "${theme.slug}" has no CSS`);
