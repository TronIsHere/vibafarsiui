...
import { readText, resolveTarget, writeFile } from "./project.js";
import { fetchItem, resolveOne } from "./registry.js";
/** Guides copied into docs/ on init. Both are served by the registry as skills. */
export const INIT_GUIDES = ["agents-md-persian", "ui-craft-rules"];
 ...
 */
export function agentsBlock(font) {
...
function ensureClaudeImport(project, dryRun) {
...
async function copyGuide(project, flags, client, catalog, slug) {
    const item = resolveOne(catalog, slug);
    if (!item || item.type !== "skill")
        throw new Error(`Registry is missing guide "${slug}"`);
    const payload = await fetchItem(client, item);
    const file = payload.files[0];
    if (!file)
        throw new Error(`Registry guide "${slug}" has no files`);
 ...
 */
export async function writeAgentGuides(project, flags, client, catalog, font) {
    ...
    if (ensureClaudeImport(project, flags.dryRun))
        info(`CLAUDE.md  ${CLAUDE_IMPORT}`);
    else
