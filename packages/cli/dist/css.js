import { rewriteUserSource } from "./registry.js";
import { readText, writeFile } from "./project.js";
const START = (id) => `/* --- vibefarsi:${id} --- */`;
const END = (id) => `/* --- vibefarsi:${id}-end --- */`;
export function upsertBlock(source, id, body) {
...
export function hasBlock(source, id) {
...
export function fontTheme(variable) {
    return `@theme inline {
...
export function applyItemCss(project, item, dryRun) {
    if (!item.css?.trim())
        return false;
    return applyCss(project, item.name, item.css, dryRun);
}
export function applyThemeTokens(project, themeSlug, css, fontVar, dryRun, opts = {}) {
